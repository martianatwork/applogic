...
module Peatio
  module Market
    class EthdashTradeCompletedConsumer
    def call(event)
        Rails.logger.info "Received trade event #{event}"
        if event[:buyer_income_fee].to_f > 0
          buyer_referral = Referral.new(kind: 'referral-payout',currency: event[:buyer_income_unit],
        ...
        if event[:seller_income_fee].to_f > 0
          seller_referral = Referral.new(kind: 'referral-payout',currency: event[:seller_income_unit],
